...
impl GsuiteClient {
    ...
    pub async fn run_function(&self, function: &str, parameters: Vec<serde_json::Value>) -> Result<GscriptResponse> {
        let url = format!(
        ...
        Ok(GscriptResponse {
            ...
            error: data.get("error").and_then(|e| e.get("message")).and_then(|m| m.as_str()).map(|s| s.to_string()),
        })
    }

    pub async fn create_document(&self, title: &str) -> Result<String> {
        let result = self.run_function("createDocument", vec![serde_json::json!(title)]).await?;
        result.result
            .and_then(|r| r.as_str().map(|s| s.to_string()))
            .ok_or_else(|| anyhow::anyhow!("Failed to create document"))
    }

    pub async fn append_to_sheet(&self, spreadsheet_id: &str, values: Vec<Vec<String>>) -> Result<()> {
        let values_json: Vec<serde_json::Value> = values.into_iter()
            .map(|row| serde_json::json!(row))
            .collect();

        self.run_function(
            "appendToSheet",
            vec![serde_json::json!(spreadsheet_id), serde_json::json!(values_json)],
        ).await?;
