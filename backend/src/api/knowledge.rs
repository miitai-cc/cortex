...
async fn overview(
    ...
    let point_events=sqlx::query("SELECT points,reason,target_id,CAST(created_at AS TEXT) AS created_at FROM knowledge_point_events WHERE user_id=? ORDER BY created_at DESC").bind(&identity.sub).fetch_all(&state.db.pool).await?.into_iter().map(|r|serde_json::json!({"points":r.get::<i64,_>("points"),"reason":r.get::<String,_>("reason"),"targetId":r.try_get::<String,_>("target_id").ok(),"createdAt":r.try_get::<String,_>("created_at").ok()})).collect::<Vec<_>>();
...
async fn comments(
    ...
    let id = req
        .param::<String>("id")
        .ok_or_else(|| AppError::BadRequest("Missing id".into()))?;
    let rows=sqlx::query("SELECT id,user_id,username,content,parent_id,is_best,CAST(created_at AS TEXT) AS created_at FROM knowledge_comments WHERE record_id=? ORDER BY created_at").bind(id).fetch_all(&state.db.pool).await?;
