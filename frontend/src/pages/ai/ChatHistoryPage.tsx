...
export default function ChatHistoryPage() {
  ...
  return (
    <div>
      <CommonHeroTitle icon={Clock} title={t('nav.chat.history')} description="瀏覽與管理過往的對話紀錄" />

      {sorted.length === 0 ? (
        <div className="card text-center py-12">
          <MessageSquare className="w-12 h-12 text-gray-300 dark:text-gray-600 mx-auto mb-3" />
          <p className="text-sm text-gray-500 dark:text-gray-400">尚無對話紀錄</p>
          <button
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500">
                  {conv.messages.length} 則訊息 · {new Date(conv.updatedAt).toLocaleString()}
                </p>
              </div>
              <button
