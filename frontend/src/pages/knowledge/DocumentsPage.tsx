...
export default function DocumentsPage() {
  ...
  return (
    <div className="max-w-11xl mx-auto px-4 pb-10">
      ...
      <p className="mb-3 text-sm text-gray-500 dark:text-gray-400">{t("documents.operatingDirectory")}<span className="font-medium text-primary-600">{directory}</span></p>
      ...
      {indexEvents.length > 0 && (
        <section className="card mb-6" aria-live="polite">
          <div className="flex items-center justify-between gap-3 mb-3">
            <div>
              <h2 className="font-semibold text-gray-900 dark:text-gray-100">{t("documents.backendIndexing")}</h2>
              <p className="text-xs text-gray-500 dark:text-gray-400">{t("documents.grpcEvents")}</p>
            </div>
            <button
              type="button"
      ...
      {isLoading ? (
        ...
        <div className="grid gap-3">
                    ...
                    {meta.page_count != null && (
                      <>
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {t("documents.pageCount", { count: meta.page_count })}
                        </span>
                      </>
                    )}
          ...
          {docs.length === 0 && (
            <p className="text-gray-400 dark:text-gray-500 text-center py-8">{t("documents.noDocuments")}</p>
          )}
        </div>
      )}
    </div>
