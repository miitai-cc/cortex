...
export default function DiscussionBoard() {
  ...
  if (isLoading) {
    return <div className="flex h-64 items-center justify-center text-gray-400">載入討論區中...</div>;
  }
  ...
  const renderAddForm = (type: 'category' | 'topic' | 'item', onCancel: () => void) => (
    <div className="mb-4 rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
      ...
      <div className="flex justify-end gap-2">
        <button className="btn-secondary" onClick={onCancel} disabled={createMutation.isPending}>取消</button>
        <button className="btn-primary" onClick={() => handleCreate(type)} disabled={createMutation.isPending || !formTitle.trim()}>
          {createMutation.isPending ? '儲存中...' : '確認新增'}
        </button>
      </div>
    </div>
  );
    ...
    <div className="flex h-[calc(100vh-14rem)] min-h-[600px] gap-6 overflow-hidden">
      ...
      <div className="flex w-64 shrink-0 flex-col overflow-y-auto rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
        <div className="sticky top-0 flex items-center justify-between border-b border-gray-100 bg-white/90 p-4 backdrop-blur dark:border-gray-800 dark:bg-gray-900/90 z-10">
          <h2 className="font-semibold text-gray-700 dark:text-gray-300">討論分類</h2>
        ...
        <div className="p-3">
          ...
          <div className="space-y-1">
            ...
            {categories.length === 0 && !showAddForm && <div className="p-4 text-center text-sm text-gray-400">目前沒有分類</div>}
          </div>
        </div>
      </div>
          ...
          <>
            <div className="sticky top-0 flex items-center justify-between border-b border-gray-100 bg-white/90 p-4 backdrop-blur dark:border-gray-800 dark:bg-gray-900/90 z-10">
              <h2 className="font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-2">
                ...
                {categories.find(c => c.id === selectedCategory)?.title} - 議題列表
              </h2>
                ...
                {topics.map(topic => (
                  <div key={topic.id} onClick={() => { setSelectedTopic(topic.id); setShowAddForm(null); }} className="group cursor-pointer rounded-xl border border-gray-200 p-4 transition-all hover:border-primary-300 hover:shadow-sm dark:border-gray-800 dark:hover:border-primary-700">
                    ...
                    <div className="mt-3 flex items-center gap-3 text-xs text-gray-400">
                      <span className="flex items-center gap-1"><MessageSquare className="h-3.5 w-3.5" /> {items.filter(i => (i.metadata as any)?.topicId === topic.id).length} 個項目</span>
                      <span>{formatRelativeTime(topic.createdAt)}</span>
                    </div>
                  </div>
                ))}
                {topics.length === 0 && !showAddForm && <div className="col-span-full py-10 text-center text-sm text-gray-400">尚無議題，點擊右上角新增</div>}
              </div>
            </div>
          </>
        ) : !selectedItem ? (
          <>
            <div className="sticky top-0 flex items-center justify-between border-b border-gray-100 bg-white/90 p-4 backdrop-blur dark:border-gray-800 dark:bg-gray-900/90 z-10">
              ...
              <button onClick={() => setShowAddForm('item')} className="btn-primary flex items-center gap-1.5 py-1.5 text-xs">
                <Plus className="h-3.5 w-3.5" /> 新增討論項目
              </button>
            </div>
            <div className="p-4">
                ...
                {topicItems.map(item => (
                  <div key={item.id} onClick={() => { setSelectedItem(item.id); setShowAddForm(null); }} className="cursor-pointer rounded-xl border border-gray-100 bg-gray-50/50 p-4 transition-all hover:border-primary-200 hover:bg-white dark:border-gray-800 dark:bg-gray-800/20 dark:hover:border-primary-800 dark:hover:bg-gray-800">
                    ...
                    <div className="mt-4 flex items-center gap-4 text-xs text-gray-500">
                      ...
                      <span className="flex items-center gap-1"><MessageCircle className="h-3.5 w-3.5" /> {items.filter(i => i.itemType === 'comment' && (i.metadata as any)?.itemId === item.id).length} 則留言</span>
                      <span>{formatDateTime(item.createdAt)}</span>
                    </div>
                  </div>
                ))}
                {topicItems.length === 0 && !showAddForm && <div className="py-10 text-center text-sm text-gray-400">還沒有人發起討論，來當第一個吧！</div>}
              </div>
            </div>
          </>
        ) : (
          <div className="flex h-full flex-col">
            <div className="sticky top-0 flex items-center justify-between border-b border-gray-100 bg-white/90 p-4 backdrop-blur dark:border-gray-800 dark:bg-gray-900/90 z-10">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-semibold text-gray-700 dark:text-gray-300">項目討論</h2>
              </div>
            </div>
                ...
                return (
                  <div className="mb-8">
                    ...
                    <div className="mb-4 flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-sm font-medium text-gray-700 dark:text-gray-300">{item.ownerName || '使用者'}</div>
                  ...
                  <div key={comment.id} className="flex gap-3">
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex items-baseline gap-2">
                        <span className="text-sm font-medium text-gray-800 dark:text-gray-200">{comment.ownerName || '使用者'}</span>
            ...
            <div className="border-t border-gray-100 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-900">
              <div className="flex gap-3">
                ...
                >
                  <Reply className="h-4 w-4" /> {createMutation.isPending ? '送出中' : '送出'}
                </button>
              </div>
            </div>
          </div>
